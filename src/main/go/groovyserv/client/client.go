...
var (
	HOME                = my.HomeDir()
	GROOVYSERV_WORK_DIR = my.GroovyServWorkDir()
	GROOVYSERV_HOME     = my.Env("GROOVYSERV_HOME", filepath.Join(path.Dir(os.Args[0]), ".."))
...
func NewOptions() Options {
	return Options{
		client: ClientOptions{
			host:            my.Env("GROOVYSERV_HOST", DEFAULT_HOST),
			port:            my.EnvInt("GROOVYSERV_PORT", DEFAULT_PORT),
			authToken:       "",
			quiet:           false,
			envAll:          false,
			envIncludeMask:  []string{},
...
func ParseOptions(args my.Args) Options {
	opts := NewOptions()
...
func StartSession(opts Options) {
	...
	conn, err := my.Connect(opts.client.host, opts.client.port)
	if err != nil {
...
func WriteStreamRequest(writer io.Writer) {
	...
	for {
		...
		if err != nil {
			my.Write(writer, "Size: 0\n\n")
			break
		}
		my.Write(writer, fmt.Sprintf("Size: %d\n\n", len(input)))
		my.Write(writer, input) // without LF
	}
}
...
func WriteInvocationRequest(writer io.Writer, opts Options) {
	my.WriteLine(writer, "Cwd: /tmp")
	my.WriteLine(writer, "Auth: "+AuthToken(opts.client.port, opts))
	for _, arg := range opts.server.args {
		my.WriteLine(writer, "Arg: "+b64.StdEncoding.EncodeToString([]byte(arg)))
	}
	WriteEnv(writer, opts)
	if cp := my.Env("CLASSPATH", ""); cp != "" {
		my.WriteLine(writer, "Cp: "+cp)
	}
	my.WriteLine(writer, "")
}
...
func WriteEnv(writer io.Writer, opts Options) {
	for _, env := range os.Environ() {
		...
		if opts.client.envAll || my.ArrayFuncAny(opts.client.envIncludeMask, maskMatcher) {
			if !my.ArrayFuncAny(opts.client.envExcludeMask, maskMatcher) {
				my.WriteLine(writer, "Env: "+env)
			}
		}
	}
}
...
func StartServer(opts Options) {
	...
	if !my.FileExists(serverCmdFile) {
...
func Run() {
	...
	if my.ArrayAny(opts.client.startServerOpts, "-k", "-r") {
		StartServer(opts)
		if my.ArrayAny(opts.client.startServerOpts, "-k") {
			os.Exit(0)
		}
	}
