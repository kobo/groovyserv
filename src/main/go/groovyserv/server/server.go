...
var (
	...
	GROOVYSERV_HOME     = my.GroovyServHome(os.Args[0])
	GROOVY_HOME         = my.Env("GROOVY_HOME", "")
	GROOVYSERV_OPTS     = my.Env("GROOVYSERV_OPTS", "")
)
...
func GroovyCommand(opts Options) string {
	...
	if len(GROOVY_HOME) > 0 {
		...
		if !my.FileExists(groovyCmdPath) {
	...
	if my.CommandExists(groovyCmdName) {
...
func StartServer(opts Options) {
	...
	if len(classpathEnv) > 0 {
		classpath = classpath+my.ClasspathDelimiter()+classpathEnv
	}
	printConsole("Original classpath: "+my.Coalesce(classpathEnv, "(none)"), opts)
	...
	javaOpts := "-server -Djava.awt.headless=true " + my.Env("JAVA_OPTS", "")
