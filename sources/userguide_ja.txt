...
groovyclientでは-Cで始まる以下の起動オプションが指定可能です。これらのオプションはgroovyclientで解釈され、groovyコマンドには渡されません。::
  ...
  -Ck,-Ckill-server             起動中のgroovyserverを停止する (groovyclient.shでは使えません)
  -Cr,-Crestart-server          起動中のgroovyserverを再起動する (groovyclient.shでは使えません)
...
ただし、シェルスクリプトの機能の限界により、一部の機能に制限があります。
 ...
 - 透過的サーバ起動機能の内、再起動と停止が使えません (直接groovyserverコマンドで操作してください)
...
機能制限のないクライアントを利用したい場合は、フル機能サポートしているruby版のgroovyclient.rbを利用するか、または、自前でネイティブバイナリをビルドしてご利用ください。
